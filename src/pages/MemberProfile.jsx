        ...
        {member && (
          <>
            ...
            <div className="card flex flex-col sm:flex-row gap-6 items-start mb-5">
              <div className="flex-shrink-0">
                {member.profile_photo ? (
                  <img src={member.profile_photo} alt={member.full_name}
                    className={`w-24 h-24 rounded-2xl object-cover ring-2 ring-slate-700 ${member.is_deceased ? 'grayscale opacity-80' : ''}`} />
                ) : (
                  <div className={`w-24 h-24 rounded-2xl flex items-center justify-center text-white font-bold text-2xl ${member.is_deceased ? 'bg-slate-600' : 'bg-brand-700'}`}>
                    {initials}
                  </div>
                )}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-start gap-2 flex-wrap">
                  ...
                  {member.is_deceased && (
                    <span className="badge bg-slate-700 text-slate-400 border border-slate-600 mt-1">† Deceased</span>
                  )}
                </div>
                {member.nickname && (
                  <p className="text-slate-400 text-sm italic mt-0.5">"{member.nickname}"</p>
                )}
                {member.occupation && (
                  <p className="text-slate-500 text-sm mt-1">💼 {member.occupation}</p>
                )}
                {/* Interests chips */}
                {interests.length > 0 && (
            ...
            <div className="card mb-5">
              <h2 className="text-sm font-semibold text-slate-300 mb-4">Personal Details</h2>
            ...
            {member.biography && (
              <div className="card mb-5">
                <h2 className="text-sm font-semibold text-slate-300 mb-3">Biography</h2>
                <p className="text-slate-400 text-sm leading-relaxed whitespace-pre-line">{member.biography}</p>
              </div>
            )}
            ...
            {member.custom_notes && (
              <div className="card mb-5">
                <h2 className="text-sm font-semibold text-slate-300 mb-3">Notes</h2>
                <p className="text-slate-400 text-sm leading-relaxed whitespace-pre-line">{member.custom_notes}</p>
              </div>
            )}
            ...
            {hasSocials && (
              <div className="card mb-5">
                <h2 className="text-sm font-semibold text-slate-300 mb-4">Social Media</h2>
                    ...
                    .map(([key, val]) => {
                      ...
                      return (
                        <a
                          ...
                          className="flex items-center gap-2 px-3 py-2 rounded-xl bg-slate-800 border border-slate-700 hover:border-brand-600 text-slate-300 hover:text-white text-sm transition-all"
                        >
                          <span>{meta.icon}</span>
                          <span>{val}</span>
                        </a>
            ...
            <div className="card mb-5">
              ...
              {gallery.length === 0 ? (
                <div
                  onClick={() => galleryInputRef.current?.click()}
                  className="flex flex-col items-center justify-center h-28 border-2 border-dashed border-slate-700 hover:border-brand-600 rounded-xl cursor-pointer text-slate-500 hover:text-brand-400 transition-colors"
                >
                ...
                <div className="grid grid-cols-3 gap-2">
                  {gallery.map((photo) => (
                    <div
                      key={photo.id}
                      className="relative group cursor-pointer rounded-xl overflow-hidden aspect-square bg-slate-800"
                    ...
                    >
                      ...
                      {photo.uploaded_by === user?.id && (
                        <button
                          ...
                          className="absolute top-1.5 right-1.5 w-6 h-6 bg-slate-900/80 hover:bg-red-700 text-white rounded-lg text-xs flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
                        >×</button>
                      )}
                    </div>
                  ))}
      ...
      {lightbox && (
        ...
        >
          <div className="relative max-w-3xl w-full" onClick={(e) => e.stopPropagation()}>
            ...
            <button
              onClick={() => setLightbox(null)}
              className="absolute top-3 right-3 w-8 h-8 bg-slate-900/80 hover:bg-red-900 text-white rounded-full flex items-center justify-center text-lg transition-colors"
            >×</button>
          </div>
        </div>
      )}
